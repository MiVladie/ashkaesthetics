...
import * as classes from './Catalog.module.scss';
...
const Catalog = ({ data, maxVisible, expandText, collapseText, linkTo, linkText }) => {
	...
	function timeConverter(time) {
		...
		if (Math.trunc(time / 60) > 1) result += 's';
				...
				<Animation>
					<div className={classes.Line}>
						...
						<p className={classes.Price}>
							{item.oldPrice && <span className={classes.Discounted}>£{item.oldPrice}</span>} £
							{item.price}
						</p>
					</div>
	...
	return (
		<div className={classes.Catalog}>
			<ul className={classes.List}>{itemsList()}</ul>

			<Animation>
				<div className={classes.Options}>
					{maxVisible && maxVisible < data.length && (
					...
					{linkTo && (
						<a className={classes.Link} href={linkTo} target='_blank' rel='noopener noreferrer'>
							{linkText}
						</a>
					)}
				</div>
